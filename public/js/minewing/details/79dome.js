...
async function scrapeProduct(page, url) {
    try {
        ...
        const productData = await page.evaluate(() => {
            const productName = document.querySelector('#goods_spec > form > div > b').textContent.trim();
            const productPrice = document.querySelector('#price > font > b').textContent.trim().replace(/[^\d]/g, '');
            const productImage = document.querySelector('[id="objImg"]').src;
