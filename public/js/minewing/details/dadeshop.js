...
async function getProductDetail(page) {
    return await page.evaluate(() => {
        ...
        const excludedPatterns = [
            'haven96.imghost.cafe24.com/1dade/DADE-INFO.jpg',
            'haven96.imghost.cafe24.com/1dade/DADE-INFO1.jpg'
        ];
        if (productDetailElements.length > 0) {
            return Array.from(productDetailElements)
                .map(element => element.src)
                .filter(src => !excludedPatterns.some(pattern => src.includes(pattern)));
        }
        return false;
    });
}
...
async function getProductImage(page) {
    const productImage = await page.evaluate(() => {
        return document.querySelector('div.xans-element-.xans-product.xans-product-image.imgArea > div.keyImg > div > a > img').src;
    });
    return productImage;
}
...
async function getProductName(page) {
    const productName = await page.evaluate(() => {
        const productNameElement = document.querySelector('div.detailArea > div.infoArea > div.headingArea > h2');
        let productNameText = productNameElement.textContent.trim();

        // "해외배송"을 포함하는 괄호와 내용을 제거합니다.
        productNameText = productNameText.replace(/[A-Za-z]{2}-[A-Za-z0-9]+/g, '');
        productNameText = productNameText.replace('상품명 :', '');
