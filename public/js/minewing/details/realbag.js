...
async function getProductName(page) {
    const productName = await page.evaluate(() => {
        const productNameElement = document.querySelector('div.prdTitle > h3');
        if (!productNameElement) {
            return false;
        }

        // Remove non-product name info
        const spanTags = productNameElement.querySelectorAll('span');
        spanTags.forEach(span => span.remove());

        const productNameText = productNameElement.textContent;

        // Check for sold out
        if (productNameText.includes('품절')) {
            return false;
        }

        // Get the text content up to the first <br> tag and clean it
        let productName = productNameElement.innerHTML.split('<br>')[0].trim();
        productName = productName.replace(/<\/?[^>]+(>|$)/g, ""); // Remove any remaining HTML tags
...
async function getproductImage(page) {
    const imageUrl = await page.evaluate(() => {
        const productImageElement = document.querySelector('div.keyImg > img');
