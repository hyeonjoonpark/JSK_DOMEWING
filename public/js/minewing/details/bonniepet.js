...
(async () => {
    ...
    try {
        ...
        // const urls = ['http://bonniepet.co.kr/goods/goods_view.php?goodsNo=1000018490'];
...
async function scrapeProduct(page, productHref) {
    try {
        // 제품 이미지 URL을 가져옵니다. #frmView > div > div.goods-header > div.top > div > h2
        ...
        const product = {
            ...
            sellerID: 38 // 판매자 ID는 상수로 38을 할당합니다.
        };
...
async function getProductOptions(page) {
    async function reloadSelects() {
        return page.$$('select.tune');
    }

    async function resetSelects() {
        const delBtn = await page.$('#option_display_item_54227_1711354246431 > div > div.del > button');
        if (delBtn) {
            await delBtn.click();
        ...
        if (currentDepth < selects.length) {
            const options = await selects[currentDepth].$$eval('option:not(:disabled)', opts =>
                opts.map(opt => ({ value: opt.value, text: opt.text }))
                    .filter((opt, index) => index > 0 && !opt.text.includes("품절")) // 첫 번째 옵션을 건너뛰고, "품절" 텍스트가 없는 옵션만 필터링
            );

            for (const option of options) {
                await selects[currentDepth].select(option.value);
                await page.waitForSelector('#selectId option[value="optionValue"]', { visible: true });
