...
async function scrapeProduct(page, productHref) {
    try {
        ...
        if (productImage.includes('div.prdImg > div.thumbnail > a > img')) {
            return false;
        }
...
async function getProductImage(page) {
    const productImage = await page.evaluate(() => {
        return document.querySelector('div.prdImg > div.thumbnail > a > img').src;
    });
    return productImage;
}
...
async function getProductName(page) {
    const productName = await page.evaluate(() => {
        const productNameElement = document.querySelector('div.detailArea > div.infoArea > div.headingArea > h1');
