...
async function getProductName(page) {
    const productName = await page.evaluate(() => {
        const productNameElement = document.querySelector('#contents > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.buy-wrapper > div.buy-scroll-box > h2');
    ...
    // 괄호 안의 문자열을 제거하는 정규식 적용
    const formattedProductName = productName.replace(/\(.*?\)/g, '').trim();

    return await formatProductName(formattedProductName);
}

async function getproductPrice(page) {
    return await page.evaluate(() => {
        const salePriceElement = document.querySelector('#contents > div.xans-element-.xans-product.xans-product-detail > div.detailArea > div.buy-wrapper > div.buy-scroll-box > div.infoArea.DB_rate > div.xans-element-.xans-product.xans-product-detaildesign > table > tbody > tr.price.xans-record- > td > span > strong');
        if (!salePriceElement) {
            return false;
        }
