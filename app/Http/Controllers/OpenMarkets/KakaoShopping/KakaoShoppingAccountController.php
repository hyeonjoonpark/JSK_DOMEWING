...
namespace App\Http\Controllers\OpenMarkets\KakaoShopping;
...
class KakaoShoppingAccountController extends Controller
{
    ...
    {
        try {
            DB::table('kakao_shopping_accounts')
                ->insert([
    ...
    public function requestValidateApiKey($accessKey)
    {
        ...
        $ac = new KakaoShoppingAccountController();
    ...
    public function edit(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'accessKey' => ['required', 'max:255'],
            'username' => ['required', 'max:255'],
            'hash' => ['required', 'exists:kakao_shopping_accounts,hash']
        ], [
            'accessKey.required' => 'OPEN API KEY는 필수 항목입니다.',
            'accessKey.max' => 'OPEN API KEY는 최대 255자 이하로 입력해야 합니다.',
            'username.required' => '계정명 및 별칭은 필수 항목입니다.',
